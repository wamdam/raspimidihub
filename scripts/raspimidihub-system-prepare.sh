...
[Service]
# Pin the asyncio process exclusively to the kernel-isolated CPU 3.
# Combined with isolcpus=3 nohz_full=3 rcu_nocbs=3 in the kernel
# cmdline, this gives the Python main loop a dedicated core that
# nothing else can preempt.
AllowedCPUs=3
# Slight nice bump — the loop is the most latency-sensitive thing on
# the Pi.
Nice=-5
EOF
