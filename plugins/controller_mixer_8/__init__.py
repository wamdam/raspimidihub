...
Fixed 8-wide layout: 2 rows of knobs / 8 vertical faders / 8 mute
buttons (4 rows total). Defaults on ch 1; user can override per cell
...
class ControllerMixer8(ControllerBase):
    ...
    DESCRIPTION = "8-wide mixer: 16 knobs / 8 faders / 8 mute buttons (defaults CC 16-47 ch 1)"
    AUTHOR = "RaspiMIDIHub"
    VERSION = "1.1"
    HELP = """\
8-wide mixer-strip controller. Four rows of 8 cells with default
bindings on channel 1:
  - Row 1: knobs row A (K1..K8)  -> CC 16..23
  - Row 2: knobs row B (K9..K16) -> CC 24..31
  - Row 3: vertical faders        -> CC 32..39
  - Row 4: mute buttons           -> CC 40..47 (0 / 127)
...
keep both sides in sync without feedback loops."""
    ...
    cc_outputs = list(range(16, 48))  # CC 16..47 ch 1
    outputs = ["CC 16..47 on ch 1 — 16 knobs, 8 faders, 8 mute buttons"]
