...
make deb           # builds dist/raspimidihub_1.1.2-1_all.deb
make deb-rosetup   # builds dist/raspimidihub-rosetup_1.1.2-1_all.deb
```
...
1. `Makefile` — `VERSION = 1.1.2` (used for .deb filename and control file)
2. `src/raspimidihub/__init__.py` — `__version__ = "1.1.2"` (reported in web UI)
