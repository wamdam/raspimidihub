...
def _topic_id(entries: list) -> str:
    """Stable id derived from the SOURCE entries, never the LLM title.

    The title is regenerated by the LLM on every run and is never
    byte-identical, so hashing it (the old behaviour) produced a fresh id each
    time and defeated dedup entirely — the cause of the repeated link-local /
    runs-on-any-computer posts. Keying on the sorted member-entry hashes means
    the same cluster maps to the same id across re-clusterings. This is the
    project rule: hash the source, not the output.
    """
    joined = '|'.join(sorted(e['key'] for e in entries))
    return _key(joined) if joined else _key('empty-topic')
...
class FeaturesSource(Source):
    ...
    def _cluster_topics(self, llm) -> list:
        ...
        # Cluster the FULL changelog, not just the newest slice. The old
        # [:50] cap meant only the current dev cycle (link-local, MIDI 2.0,
        # runs-on-any-computer, WiFi) was ever eligible, which is why the
        # feature posts felt one-note. The whole history is the pool now.
        ...
        try:
            out = llm.generate(_CLUSTER_SYSTEM, user, temperature=0.2, max_tokens=4000)
        except Exception:
            out = None
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                for title, desc in data.items():
                    ...
                    entries_for_topic = matching_entries[:5]  # Limit per topic
                    validated.append({
                        'id': _topic_id(entries_for_topic),  # source-based, not title
                        'title': title,
                        'description': desc if isinstance(desc, str) else '',
                        'entries': entries_for_topic,
                    })
    ...
    def latest(self) -> list:
        ...
        if items:
            return [{'id': _topic_id([items[0]]), 'title': items[0]['text'][:50],
                     'entries': [items[0]]}]
        return []
